...
interface TestCase {
  ...
  description: string;
  input: {
    inputSOL: number;              // 输入SOL数量（lamports）
    historicalRatio: number;       // 历史比率（SOL → USDC）
    actualOutUSDC: number;         // 去程实际输出（USDC，最小单位）
    estimatedReturnSOL: number;    // 回程估算输出（SOL lamports，基于估算USDC）
  };
  expected: {
    profit: number;                // 期望利润（SOL lamports）
    profitSOL: number;             // 期望利润（SOL）
    accuracy: string;              // 准确性描述
    shouldExecute: boolean;        // 是否应该执行套利
  };
}
...
const testCases: TestCase[] = [
  ...
  // ========================================================================
  {
    ...
    expected: {
      profit: 527_567_567,                // ~0.53 SOL（仍有利润）
      profitSOL: 0.53,
      accuracy: "中低精度（5%波动）",
...
interface CalculationResult {
  estimatedBridgeAmount: number;    // 估算的桥接代币数量
  pricePerBridge: number;           // 单价（SOL/USDC）
  actualReturnSOL: number;          // 实际回程SOL
  profit: number;                   // 利润（lamports）
  profitSOL: number;                // 利润（SOL）
  roi: number;                      // ROI（%）
  estimationError: number;          // 估算误差（%）
  newRatio: number;                 // 更新后的比率
}

function calculateWithUnitPrice(
  inputSOL: number,
  historicalRatio: number,
  actualOutUSDC: number,
  estimatedReturnSOL: number
): CalculationResult {
  // 步骤1: 估算去程输出（USDC）
