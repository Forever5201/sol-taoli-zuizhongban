...
export class RiskManager {
  private readonly profitAnalyzer: ProfitAnalyzer;

  constructor(profitAnalyzer?: ProfitAnalyzer) {
    this.profitAnalyzer = profitAnalyzer || new ProfitAnalyzer();
  }
  ...
  private getFailureReason(
    ...
    if (!checks.profitThreshold) {
      reasons.push(
        `利润不足: ${analysis.netProfit} < ${config.minProfitThreshold} lamports`
      );
    }
    ...
    if (!checks.costLimit) {
      if (analysis.costs.priorityFee > config.maxGasPrice) {
        reasons.push(
          `优先费过高: ${analysis.costs.priorityFee} > ${config.maxGasPrice} lamports`
        );
      }
      if (analysis.costs.jitoTip > config.maxJitoTip) {
        reasons.push(
          `Jito 小费过高: ${analysis.costs.jitoTip} > ${config.maxJitoTip} lamports`
        );
      }
    }
    ...
    if (!checks.slippage) {
      reasons.push(
        `滑点过大: ${(opportunity.estimatedSlippage * 100).toFixed(2)}% > ${(config.maxSlippage * 100).toFixed(2)}%`
      );
    }

    if (!checks.liquidity) {
      reasons.push(
        `流动性不足: $${opportunity.poolLiquidity.toFixed(0)} < $${config.minLiquidity.toFixed(0)}`
      );
    }

    if (!checks.roi) {
      reasons.push(
        `ROI 过低: ${analysis.roi.toFixed(2)}% < ${config.minROI}%`
      );
    }
