...
export class ProfitAnalyzer {
  private static readonly DEFAULT_SLIPPAGE_BUFFER = 1.2;

  private readonly slippageBuffer: number;
  private readonly conservativeEstimate: boolean;
  ...
  evaluateMultipleOpportunities(
    ...
    costConfig: CostConfig,
    jitoTip: number
  ): Array<{ opportunity: ArbitrageOpportunity; analysis: ProfitAnalysis }> {
  ...
  getBestOpportunity(
    ...
    minProfitThreshold: number,
    minROI: number
  ): { opportunity: ArbitrageOpportunity; analysis: ProfitAnalysis } | null {
    const evaluated = this.evaluateMultipleOpportunities(
      opportunities,
      costConfig,
      jitoTip
    );
    ...
    const best = evaluated.find(
      (item) =>
        this.shouldExecute(item.analysis, minProfitThreshold, minROI)
    );
  ...
  generateReport(analysis: ProfitAnalysis): string {
    ...
    lines.push(
      `净利润: ${netProfit > 0 ? '+' : ''}${netProfit} lamports`,
      `ROI: ${roi.toFixed(2)}%`,
      `成本占比: ${formatPercentage(costRatio)}`,
      `结论: ${analysis.isProfitable ? '✅ 盈利' : '❌ 亏损'}`,
      '================================='
    );
  ...
  simulateProfitCurve(
    ...
    steps: number = 10
  ): Array<{ tip: number; netProfit: number; roi: number }> {
    const [minTip, maxTip] = tipRange;
    const stepSize = (maxTip - minTip) / (steps - 1);

    const curve: Array<{ tip: number; netProfit: number; roi: number }> = [];
  ...
  calculateMaxAffordableTip(
    ...
    // 计算不含 Jito 小费的成本
    const costsWithoutTip = CostCalculator.calculateTotalCost(costConfig, 0);
