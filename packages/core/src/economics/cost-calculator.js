...
class CostCalculator {
    static DEFAULT_RPC_COST = 100; // lamports
    /**
     * 计算基础交易费
     * @param signatureCount 签名数量
     * @returns 基础费用（lamports）
     */
    static calculateBaseFee(signatureCount) {
        return signatureCount * types_1.BASE_FEE_PER_SIGNATURE;
    }
    /**
     * 计算优先费
     ...
     */
    static calculatePriorityFee(computeUnits, computeUnitPrice) {
     ...
     * @returns 估算的计算单元数量
     */
    static estimateComputeUnits(config) {
     ...
     */
    static calculateFlashLoanFee(loanAmount) {
        return Math.ceil(loanAmount * types_1.FLASH_LOAN_FEE_RATE);
    }
    /**
     * 计算交易总成本
     * @param config 成本配置
     ...
     * @returns 完整的成本明细
     */
    static calculateTotalCost(config, jitoTip) {
        // 1. 基础交易费
        const baseFee = this.calculateBaseFee(config.signatureCount);
        // 2. 优先费
        const computeUnits = this.estimateComputeUnits(config);
        const priorityFee = this.calculatePriorityFee(computeUnits, config.computeUnitPrice);
        // 3. RPC 成本
        ...
        // 6. 构建成本明细（用于调试和日志）
        const breakdown = {
            baseFee: (0, types_1.formatLamportsToSOL)(baseFee),
            priorityFee: (0, types_1.formatLamportsToSOL)(priorityFee),
            jitoTip: (0, types_1.formatLamportsToSOL)(jitoTip),
            rpcCost: (0, types_1.formatLamportsToSOL)(rpcCost),
            ...(flashLoanFee && { flashLoanFee: (0, types_1.formatLamportsToSOL)(flashLoanFee) }),
            total: (0, types_1.formatLamportsToSOL)(total),
        };
        return {
            baseFee,
            priorityFee,
     ...
     */
    static calculateMinProfitThreshold(config, jitoTip) {
     ...
     */
    static quickEstimate(signatureCount, computeUnits, computeUnitPrice, jitoTip) {
        const baseFee = signatureCount * types_1.BASE_FEE_PER_SIGNATURE;
     ...
     */
    static compareCosts(config1, config2, jitoTip) {
