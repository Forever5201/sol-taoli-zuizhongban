...
exports.JitoTipOptimizer = void 0;
...
class JitoTipOptimizer {
    static DEFAULT_API_URL = 'https://bundles.jito.wtf/api/v1/bundles';
    static DEFAULT_CACHE_DURATION = 10_000; // 10 seconds
    static DEFAULT_HISTORY_SIZE = 100;
    apiUrl;
    cacheDuration;
    historySize;
    // 缓存的小费数据
     ...
     * @returns 小费数据
     */
    async fetchRealtimeTipFloor(forceRefresh = false) {
        const now = Date.now();
        // 检查缓存
        if (!forceRefresh &&
            this.cachedTipData &&
        ...
        try {
            const response = await axios_1.default.get(`${this.apiUrl}/tip_floor`, { timeout: 5000 });
     ...
     */
    async getTipAtPercentile(percentile) {
        ...
        return Math.ceil(tipInSOL * types_1.LAMPORTS_PER_SOL);
    }
    /**
     * 计算竞争强度评分
     * @param metrics 竞争指标
     ...
     */
    async calculateOptimalTip(expectedProfit, competition, urgency, capitalSize) {
     ...
     */
    async getRecommendedTip(tokenPair, desiredSuccessRate = 0.7) {
        const history = this.bundleHistory.get(tokenPair) || [];
     ...
     * @returns 百分位
     */
    static getRecommendedPercentile(capitalSize) {
        switch (capitalSize) {
            case 'small':
...
exports.JitoTipOptimizer = JitoTipOptimizer;
//# sourceMappingURL=jito-tip-optimizer.js.map