...
export class CircuitBreaker {
  private static readonly DEFAULT_COOLDOWN_PERIOD = 5 * 60 * 1000; // 5 minutes
  private static readonly DEFAULT_HALF_OPEN_ATTEMPTS = 3;

  private readonly config: Required<ExtendedCircuitBreakerConfig>;
  ...
  generateStatusReport(): string {
    ...
    if (status === 'half-open') {
      lines.push(
        '',
        `🧪 测试进度: ${this.halfOpenAttempts}/${this.config.halfOpenTestAttempts}`
      );
    }
