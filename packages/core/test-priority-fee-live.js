...
const priority_fee_estimator_1 = require("./src/utils/priority-fee-estimator");
const jupiter_lend_adapter_1 = require("./src/flashloan/jupiter-lend-adapter");
async function testPriorityFeeEstimatorLive() {
    ...
    // DEX 程序账户列表
    const DEX_PROGRAMS = [
        new web3_js_1.PublicKey('675kPX9MHTjS2zt1qfr1NYHuzeLXfQM9H24wFSUt1Mp8'), // Raydium AMM
        new web3_js_1.PublicKey('JUP6LkbZbjS1jKKwapdHNy74zcZ3tLUZoi5QNyVTaV4'), // Jupiter V6
        new web3_js_1.PublicKey('whirLbMiicVdio4qvUfM5KAg6Ct8VwpYzGff3uctyCc'), // Orca Whirlpool
        new web3_js_1.PublicKey('Eo7WjKq67rjJQSZxS6z3YkapzY3eMj6Xy8X5EQVn5UaB'), // Meteora
    ];
    console.log(`\n📋 DEX 程序账户（用于优先费查询）:`);
    ...
    console.log('\n✅ 测试 2: 使用 PriorityFeeEstimator 类');
    console.log('─'.repeat(70));
    const estimator = new priority_fee_estimator_1.PriorityFeeEstimator(connection, 800000);
    console.log('\n✅ PriorityFeeEstimator 实例化成功');
    const testScenarios = [
        { profit: 0.01 * web3_js_1.LAMPORTS_PER_SOL, urgency: 'low', desc: '低利润 (0.01 SOL), 低优先级' },
        { profit: 0.1 * web3_js_1.LAMPORTS_PER_SOL, urgency: 'medium', desc: '中等利润 (0.1 SOL), 中优先级' },
        { profit: 1 * web3_js_1.LAMPORTS_PER_SOL, urgency: 'high', desc: '高利润 (1 SOL), 高优先级' },
        { profit: 5 * web3_js_1.LAMPORTS_PER_SOL, urgency: 'veryHigh', desc: '极高利润 (5 SOL), 极高优先级' },
    ];
    for (const scenario of testScenarios) {
        console.log(`\n🔸 场景: ${scenario.desc}`);
        try {
            const result = await estimator.estimateOptimalFee(scenario.profit, scenario.urgency);
            console.log(`   ✅ 估算成功!`);
            console.log(`      策略: ${result.strategy}`);
            console.log(`      每 CU 费用: ${result.feePerCU} micro-lamports`);
            console.log(`      总费用: ${(result.totalFee / web3_js_1.LAMPORTS_PER_SOL).toFixed(6)} SOL`);
    ...
    const arbitrageOpportunity = {
        inputAmount: 10 * web3_js_1.LAMPORTS_PER_SOL,
        profit: 0.05 * web3_js_1.LAMPORTS_PER_SOL,
        borrowAmount: 1000 * web3_js_1.LAMPORTS_PER_SOL,
    };
    console.log(`\n📊 套利机会:`);
    console.log(`   查询金额: ${arbitrageOpportunity.inputAmount / web3_js_1.LAMPORTS_PER_SOL} SOL`);
    console.log(`   查询利润: ${arbitrageOpportunity.profit / web3_js_1.LAMPORTS_PER_SOL} SOL`);
    console.log(`   ROI: ${(arbitrageOpportunity.profit / arbitrageOpportunity.inputAmount * 100).toFixed(2)}%`);
    console.log(`   计划借款: ${arbitrageOpportunity.borrowAmount / web3_js_1.LAMPORTS_PER_SOL} SOL`);
    ...
    console.log(`\n💰 预期利润: ${expectedProfit / web3_js_1.LAMPORTS_PER_SOL} SOL`);
    try {
        // 估算优先费
        console.log(`\n⏳ 正在动态估算优先费...`);
        const feeEstimate = await estimator.estimateOptimalFee(expectedProfit, 'high');
        console.log(`✅ 优先费估算完成: ${(feeEstimate.totalFee / web3_js_1.LAMPORTS_PER_SOL).toFixed(6)} SOL`);
        console.log(`   策略: ${feeEstimate.strategy}`);
        // 使用 JupiterLendAdapter 进行完整验证
        const validation = jupiter_lend_adapter_1.JupiterLendAdapter.validateFlashLoan(arbitrageOpportunity.borrowAmount, expectedProfit, {
            baseFee: 4 * 5000,
        ...
        if (validation.breakdown) {
            ...
            console.log(`   ├───────────────────┼──────────────┤`);
            console.log(`   │ 预期利润（毛）    │ ${(validation.breakdown.grossProfit / web3_js_1.LAMPORTS_PER_SOL).toFixed(6).padStart(12)} │`);
            console.log(`   │ - 基础交易费      │ ${(validation.breakdown.baseFee / web3_js_1.LAMPORTS_PER_SOL).toFixed(6).padStart(12)} │`);
            console.log(`   │ - 优先费（动态）  │ ${(validation.breakdown.priorityFee / web3_js_1.LAMPORTS_PER_SOL).toFixed(6).padStart(12)} │`);
            console.log(`   │ - Jito Tip        │ ${(validation.breakdown.jitoTip / web3_js_1.LAMPORTS_PER_SOL).toFixed(6).padStart(12)} │`);
            console.log(`   │ - 滑点缓冲        │ ${(validation.breakdown.slippageBuffer / web3_js_1.LAMPORTS_PER_SOL).toFixed(6).padStart(12)} │`);
            console.log(`   ├───────────────────┼──────────────┤`);
            console.log(`   │ 净利润            │ ${(validation.breakdown.netProfit / web3_js_1.LAMPORTS_PER_SOL).toFixed(6).padStart(12)} │`);
            console.log(`   └───────────────────┴──────────────┘`);
        }
        if (validation.valid) {
            console.log(`\n   ✅ 验证通过 - 这是可执行的机会！`);
            console.log(`   💰 最终净利润: ${(validation.netProfit / web3_js_1.LAMPORTS_PER_SOL).toFixed(6)} SOL`);
        }
        else {
