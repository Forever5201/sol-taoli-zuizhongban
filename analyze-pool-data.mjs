...
async function analyzeAccount(address, poolName, expectedType) {
...
async function main() {
    ...
    const meteoraInfo = await analyzeAccount(
    ...
    const raydiumInfo = await analyzeAccount(
    ...
    console.log('='.repeat(80));
    if (meteoraInfo) {
        console.log(`Meteora DLMM: ${meteoraInfo.length} 字节`);
    }
    if (raydiumInfo) {
        console.log(`Raydium CLMM: ${raydiumInfo.length} 字节`);
    }
}
